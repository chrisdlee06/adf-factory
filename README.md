...
LayerPurposeFormatBronzeRaw, unmodified copy of source tablesParquetSilverLight transformation (datetime → date conversion)DeltaGoldFully curated (PascalCase → snake_case column renaming)Delta
Tech Stack
...
Lookup — Queries sys.tables / sys.schemas to dynamically discover all SalesLT tables
ForEach — Iterates over the Lookup output and copies each table to the Bronze layer as Parquet
Bronze to Silver (Databricks Notebook) — Converts all datetime columns to date-only format
Silver to Gold (Databricks Notebook) — Renames columns from PascalCase to snake_case
